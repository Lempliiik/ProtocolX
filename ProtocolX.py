...
''' PUT NAME OF PERSON WHO YOU WANT TO DOX :) '''
username = input('\033[92m{+} Enter the name of victim: ')
...
def search():
    time.sleep(0.5)
    print(Fore.LIGHTRED_EX +"""Entered username""")
    time.sleep(0.5)
    print('wait')
    ...
    print(Fore.LIGHTMAGENTA_EX +"[+] ProtocolX is working ")
    ...
    for url in WEBSITES:
        ...
        if r.status_code == 200:
            if match == True:
                print(Fore.LIGHTYELLOW_EX +'[+] Results')
                match = False
            YELLOW(f'\n{url} - {r.status_code} - OK')
            if username in r.text:
                print(Fore.LIGHTGREEN_EX + f'[ProtocolX] MATCH: username {username} found on this url')
            else:
                print(Fore.LIGHTRED_EX + f'[ProtocolX] NOT MATCH : Username:{username} -\033[91m not found on this url.')
        count += 1

    total = len(WEBSITES)
    print(Fore.LIGHTCYAN_EX + f'FINISHED: With total of {count} count found {total} websites.')
